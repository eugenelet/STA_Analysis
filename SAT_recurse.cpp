						 ...
						 unsigned int input_count, int path_loc, vector<node*> input, vector<node*> circuit)
{
	...
	if(outcome_from_child->valid)
	{
		...
		while(1)
		{
			...
			if( next_input_set->input_count < pow2(SAT_input.size()) )
			{
				//Try a different set of input based on current node
				SAT_subpath(hierarchy, current_node, next_input_set);// set_input(path, path[path.size() - hierarchy - 1], hierarchy, 
